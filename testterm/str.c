...
void _str_realloc_len(str_t *str, int new_len) {
...
str_t str_append(str_t str, char *sz) {
    int sz_len = strlen(sz);
    _str_realloc_len(&str, str.len + sz_len);

    strncat(str.bytes, sz, sz_len);
    return str;
}

str_t str_insert(str_t str, int pos, char *sz) {
    if (pos < 0) {
        pos = 0;
    }
    if (pos > str.len) {
...
int str_split2(str_t str, char delim, str_t** ret_tokens) {
    str_t* tokens = NULL;
    int num_tokens = 0;
    ...
    for (int i=0; i < str.len; i++) {
        ...
        // Delimiter char matched.
        // Extract token string and add to tokens array.
        num_tokens++;
        if (tokens == NULL) {
            tokens = malloc(sizeof(str_t));
        } else {
            tokens = realloc(tokens, sizeof(str_t) * num_tokens);
        }
        tokens[num_tokens-1] = str_slice(str, token_start, i);
    ...
    tokens[num_tokens-1] = str_slice(str, token_start, str.len);
...
int main() {
    ...
    s1 = str_new("abc;def;ghi;jklmnop;qrstuv;wxyz");
    str_t* tokens = NULL;
    int num_tokens = str_split2(s1, ';', &tokens);
    printf("[\n");
